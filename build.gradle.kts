...
plugins {
    ...
    `maven-publish`   // this addon publishes a type other addons extend — see Landfall
}
...
// ---------------------------------------------------------------------------
// Argonautica opens an extension point (Landfall), so other addons need its type at
// COMPILE time. They get it the same way they get the host API:
//
//   build.gradle.kts:  compileOnly("net.crewco:argonautica:0.1.0")
//   addon.yml:         depends: [ Argonautica ]
//
// At RUNTIME the class comes out of the loaded Argonautica jar — the host's
// AddonClassLoader delegates to whatever an addon names in `depends:`, so there is
// exactly one Landfall class and `instanceof` works across addons. Never shade it.
//
